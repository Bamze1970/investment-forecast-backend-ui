...
function findOnemarketMatch(row, items) {
  ...
  return (
    items.find((item) => {
      ...
      // fallback за текущите 2 onemarket фонда
...
function getDisplayPrice(row, onemarketItem) {
  const price = Number(onemarketItem?.price);
  return Number.isFinite(price) ? price : row.current_price;
}

function getDisplayValue(row, onemarketItem) {
  const price = Number(onemarketItem?.price);
...
async function loadHoldings() {
  ...
  try {
    ...
    const [rows, onemarketData] = await Promise.all([
    ...
    contentView.innerHTML = `
      <section class="card">
        ...
        <p class="note">
          ...
          За onemarket фондовете се използват реалните NAV цени и дневната промяна от endpoint-а.
        </p>
        ...
        <div class="table-wrap">
          ...
          ${rows
            .map((r) => {
              ...
              const displayPrice = getDisplayPrice(r, om);
              const displayValue = getDisplayValue(r, om);
              const displayUnit = om?.currency || r.current_price_unit;
              const changeHtml = om ? onemarketBadge(om) : diffBadge(displayPrice, prev);
              const sourceDate = om?.lastUpdated
                ? `<span class="unit-muted">NAV: ${om.lastUpdated}</span>`
                : '';
    ...
    rows.forEach((r) => {
      const om = findOnemarketMatch(r, onemarketItems);
      newCache[r.product_id] = getDisplayPrice(r, om);
    });
    ...
    if (onemarketItems.length > 0) {
      setStatus('Активите са заредени успешно. Onemarket цените и промяната са обновени.');
    } else {
      setStatus('Активите са заредени успешно. Onemarket endpoint не върна данни.');
    }
  } catch (e) {
...
saveSettingsBtn.addEventListener('click', async () => {
  ...
  if (!ok) {
    setStatus('Backend не отговаря. Провери URL-а.');
    return;
  }
...
(async function init() {
  ...
  if (ok) {
    ...
    setStatus('Backend не отговаря. Отвори Настройки и провери Backend URL.');
  }
})();
